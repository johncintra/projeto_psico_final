...
type ErrorName = 'UnauthorizedError' | 'NotFoundError' | 'SavingError';
...
export async function validateToken(token: string): Promise<boolean> {
  ...
  try {
    ...
    if (!tokenExists) {
      throw { name: 'UnauthorizedError' as ErrorName, message: 'Token de acesso ausente.' };
    }
...
export async function getTestResult(
  ...
  try {
    ...
    const report = await collection.findOne(query);
    if (!report) {
      throw { name: 'NotFoundError' as ErrorName, message: `Test result with id ${id} not found!` };
    }
    const selectedLanguage =
      language ||
      (!!resultLanguages.find((l) => l.id == report.lang) ? report.lang : 'en');
...
export async function saveTest(testResult: DbResult) {
  ...
  try {
    ...
    console.error(error);
    throw { name: 'SavingError' as ErrorName, message: 'Failed to save test result!' };
  }
}
